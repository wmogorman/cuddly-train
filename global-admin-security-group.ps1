...
function Ensure-SecurityGroup {
  ...
  $existing = Get-MgGroup -All -Filter "displayName eq '$DisplayName'" -ConsistencyLevel eventual `
  ...
  $body = @{
    ...
    mailEnabled     = $false
    mailNickname    = ("globaladmins-" + (Get-Random))
  ...
  foreach ($m in $members) {
    ...
    switch ($odataType) {
      ...
      "#microsoft.graph.group" {
        ...
        foreach ($t in $tm) {
          if ($t.AdditionalProperties.'@odata.type' -eq "#microsoft.graph.user") {
...
function Sync-GroupMembersToUsers {
  ...
  foreach ($c in $current) {
    if ($c.AdditionalProperties.'@odata.type' -eq "#microsoft.graph.user") {
...
$partnerTenantIds = Get-PartnerTenantIdsFromGdap
...
foreach ($pt in $partnerTenantIds) {
  ...
  try {
    ...
    $grp = Ensure-SecurityGroup -DisplayName $TargetGroupDisplayName
    ...
    Sync-GroupMembersToUsers -GroupId $grp.Id -DesiredUserIds $desiredUsers -Prune:$PruneNonAdmins
