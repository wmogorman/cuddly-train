...
function Invoke-UserHive {
    ...
    if (Test-Path $networkKey) {
        ...
        foreach ($item in $networkItems) {
            ...
            Extract-ServersFromPath $remotePath
...
function Invoke-PerUserCleanup {
    ...
    foreach ($userProfile in $Profiles) {
        ...
        try {
            Process-UserHive -HiveRoot $hiveRoot -DisplayName $localPath
        }
        finally {
            if ($mountedHere) {
...
try {
    ...
    foreach ($line in $netUse) {
        if ($line -match '^\\\\[^\\\s]+\\[^\s]+') {
            Extract-ServersFromPath $matches[0]
        }
    }
}
catch {}
...
Write-Host "Disconnecting any active SMB connections for the current context..."
